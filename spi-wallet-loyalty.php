...
function spi_wallet_enqueue_assets() {
  // Only load assets on pages that need them, not on login/admin pages
...
if (!function_exists('spi_wallet_loyalty_activate')) {
  function spi_wallet_loyalty_activate() {
    ...
    // Update existing records to copy correo to email if email is empty
    $wpdb->query("UPDATE $tabla_clientes SET email = correo WHERE (email IS NULL OR email = '') AND correo IS NOT NULL AND correo != ''");
    ...
    // Add stamp_url column if it doesn't exist (for existing installations)
    $column_exists = $wpdb->get_results("SHOW COLUMNS FROM $tabla_config LIKE 'stamp_url'");
    if (empty($column_exists)) {
        $wpdb->query("ALTER TABLE $tabla_config ADD COLUMN stamp_url TEXT AFTER fondo_url");
    }
...
// Extend nonce lifetime for file uploads
add_filter('nonce_life', 'spi_extend_nonce_life');
function spi_extend_nonce_life($life) {
    // Extend nonce life to 2 hours for file uploads
    return 2 * HOUR_IN_SECONDS;
}

// AJAX handler for nonce refresh
add_action('wp_ajax_spi_refresh_nonce', 'spi_handle_nonce_refresh');
function spi_handle_nonce_refresh() {
    ...
    $new_nonce = wp_create_nonce($nonce_action);
...
require_once SPI_WALLET_PATH . 'includes/apple-push-service.php';
require_once SPI_WALLET_PATH . 'includes/business-notifications.php';
require_once SPI_WALLET_PATH . 'includes/business-notification-dashboard.php';
require_once SPI_WALLET_PATH . 'includes/secure-qr-access.php';
require_once SPI_WALLET_PATH . 'includes/qr-processor.php';
...
add_shortcode('app_notificaciones', 'spi_business_notifications_dashboard_shortcode');
