...
function spi_sumar_sello_callback() {
    ...
    if (function_exists('spi_generar_tarjetas_completas')) {
        spi_generar_tarjetas_completas($user_id, $cliente->nombre, $cliente->codigo_qr);
    } elseif (function_exists('spi_generar_pkpass_cliente')) {
        spi_generar_pkpass_cliente($user_id, $cliente->nombre, $cliente->codigo_qr);
    }

    // Send push notification for live update
    if (function_exists('spi_send_push_notification')) {
        spi_send_push_notification($cliente->codigo_qr, '¡Tu tarjeta ha sido actualizada! Tienes ' . $sellos_actuales . ' sellos.');
    }
    ...
    wp_send_json_success([
        'mensaje'            => '✅ Sello agregado. Tarjeta actualizada automáticamente.',
